import {FileSystem, SynchronousFileSystem, BFSOneArgCallback, BFSCallback, FileSystemOptions, BaseFileSystem} from '../core/file_system';
 ...
 */
class MirrorFile extends PreloadFile<DualAsyncMirror> implements File {
  constructor(fs: DualAsyncMirror, path: string, flag: FileFlag, stat: Stats, data: Buffer) {
...
/**
 * Configuration options for the DualAsyncMirror file system.
 */
export interface DualAsyncMirrorOptions {
...
/**
 * DualAsyncMirrorFS mirrors a synchronous filesystem into an asynchronous filesystem
 * by:
 *
 ...
 *     BrowserFS.FileSystem.DualAsyncMirror({
 ...
 * ```
 */
export default class DualAsyncMirror extends BaseFileSystem implements FileSystem {
  public static readonly Name = "DualAsyncMirror";
  ...
  /**
   * Constructs and initializes an DualAsyncMirror file system with the given options.
   */
  public static Create(opts: DualAsyncMirrorOptions, cb: BFSCallback<DualAsyncMirror>): void {
    try {
      const fs = new DualAsyncMirror(opts.local, opts.remote);
      fs._initialize((e?) => {
        if (e) {
          cb(e);
        } else {
  ...
  private _local: FileSystem;
  private _remote: FileSystem;
  private _isInitialized: boolean = false;
  private _initializeCallbacks: ((e?: ApiError) => void)[] = [];

  /**
   * **Deprecated; use DualAsyncMirror.Create() method instead.**
   *
   ...
   */
  constructor(local: FileSystem, remote: FileSystem) {
    super();
    this._local = local;
    this._remote = remote;
  }
  ...
  public getName(): string {
    return DualAsyncMirror.Name;
  }

  public _syncSync(fd: PreloadFile<any>) {
    this._local.writeFileSync(fd.getPath(), fd.getBuffer(), null, FileFlag.getFileFlag('w'), fd.getStats().mode);
    this.enqueueOp({
      apiMethod: 'writeFile',
  ...
  public supportsProps(): boolean { return this._local.supportsProps() && this._remote.supportsProps(); }
